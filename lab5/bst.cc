...
bool delete_node(Node*& root, int key) {
    ...
    // case 3: target has two children
        ...
        Node* largeleft = target->left;
        
        while(largeleft->right != NULL){
            largeleft = largeleft->right;
        }
        
        Node* itsParent=find_parent(target,largeleft);
        target->key=largeleft->key;
        
        if(itsParent->left == largeleft) {
            itsParent->left = largeleft->left;
        } else {
            itsParent->right = largeleft->left;
        }

        return true;//why need return here
    }
...
int height( Node* x ) {
    ...
    else return 1+std::max(height(x->left),height(x->right));
...
int depth( Node* root, Node* x ) {
    if(root==NULL || root==x || x==NULL) return 0;
